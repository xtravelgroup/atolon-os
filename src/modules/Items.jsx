...
function _MovimientosItem_legacy({ itemId, unidad, stockActual }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase.from("movimientos_inventario_atolon")
        .select("id, tipo, cantidad, unidad, precio_unit, fecha, usuario_email, notas, origen_tipo, anulado")
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      {movs === null ? (
        <div style={{ padding: 20, textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 11, background: B.navy, borderRadius: 10 }}>
          Cargando...
        </div>
      ) : movs.length === 0 ? (
        <div style={{ padding: 20, textAlign: "center", color: "rgba(255,255,255,0.25)", fontSize: 12, background: B.navy, borderRadius: 10 }}>
        ...
        <>
          ...
          {movs.length > 8 && (
            <button onClick={() => setExpanded(!expanded)}
              style={{ marginTop: 8, background: "transparent", border: "none", color: B.sky, fontSize: 11, cursor: "pointer", padding: "6px 0" }}>
...
// ─── Tab Cocina ───────────────────────────────────────────────────────
// Muestra todo lo que hay en LOC-ALMACEN-COCINA con filtros de categoría
// y búsqueda. Compara contra Loggro.
function InventarioCocinaTab({ items, categorias }) {
  ...
  useEffect(() => {
    ...
    supabase.from("items_stock_locacion")
      ...
      .eq("locacion_id", "LOC-ALMACEN-COCINA")
      .then(({ data }) => {
        const map = {};
        (data || []).forEach(s => { map[s.item_id] = Number(s.cantidad) || 0; });
        setStockCocina(map);
      });
  }, []);
  ...
  const filas = useMemo(() => {
    return items
      .filter(i => i.activo !== false)
      .map(i => {
        const cocina = Number(stockCocina[i.id]) || 0;
        const loggro = i.loggro_id ? Number(i.stock_actual) || 0 : null;
        const diff   = loggro != null ? cocina - loggro : null;
        return { item: i, cocina, loggro, diff };
      });
  }, [items, stockCocina]);
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 16 }}>
        {[
          { label: "Productos", value: filasFiltradas.length, color: B.sky },
          { label: "Stock Cocina", value: totalCocina.toLocaleString("es-CO"), color: "#f97316" },
          { label: "Stock Loggro", value: totalLoggro.toLocaleString("es-CO"), color: "#22c55e" },
          { label: "Con diferencia", value: conDiff, color: conDiff > 0 ? B.danger : B.success },
        ].map(k => (
          <div key={k.label} style={{ background: B.navyMid, borderRadius: 12, padding: "12px 16px", borderLeft: `4px solid ${k.color}` }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)}
          ...
          <option value="todas">Todas las categorías</option>
          {catsEnCocina.map(nombre => {
            const c = categorias.find(cc => cc.nombre === nombre);
            return (
              <option key={nombre} value={nombre}>{c?.icono || "📁"} {nombre}</option>
            );
          })}
        </select>
        <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontSize: 12, color: "rgba(255,255,255,0.7)" }}>
      ...
      <div style={{ overflowX: "auto", background: B.navyMid, borderRadius: 12 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 800 }}>
          <thead>
            <tr style={{ background: B.navyLight }}>
              {["Producto", "Categoría", "Unidad", "Cocina", "Loggro", "Diferencia"].map((h, i) => (
                <th key={h + i} style={{ padding: "11px 14px", textAlign: i < 3 ? "left" : "right", fontWeight: 700, color: "rgba(255,255,255,0.6)", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.04em" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filasFiltradas.length === 0 ? (
              <tr><td colSpan={6} style={{ padding: 30, textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 13 }}>Sin productos en Almacén Cocina.</td></tr>
            ) : filasFiltradas.map(f => {
              const cat = categorias.find(c => c.nombre === f.item.categoria);
              return (
                <tr key={f.item.id} style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                  ...
                  <td style={{ padding: "11px 14px", color: "rgba(255,255,255,0.5)", fontSize: 12 }}>{f.item.unidad || "—"}</td>
                  <td style={{ padding: "11px 14px", textAlign: "right", fontWeight: 800, color: f.cocina > 0 ? "#f97316" : f.cocina < 0 ? B.danger : "rgba(255,255,255,0.3)", fontSize: 14 }}>
                    {f.cocina.toLocaleString("es-CO")}
                  </td>
                  <td style={{ padding: "11px 14px", textAlign: "right", fontWeight: 700, color: f.loggro != null ? "#22c55e" : "rgba(255,255,255,0.3)" }}>
                    {f.loggro != null ? f.loggro.toLocaleString("es-CO") : "—"}
                  </td>
      ...
      <div style={{ marginTop: 14, padding: 12, background: B.navy, borderRadius: 8, fontSize: 11, color: "rgba(255,255,255,0.45)", lineHeight: 1.5 }}>
        ℹ️ Stock en <strong>LOC-ALMACEN-COCINA</strong>. Aquí se descuentan automáticamente los alimentos vendidos en Loggro.
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ overflowX: "auto", background: B.navyMid, borderRadius: 12 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 720 }}>
          ...
          <tbody>
            {filasFiltradas.length === 0 ? (
              <tr><td colSpan={8} style={{ padding: 30, textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 13 }}>Sin productos que coincidan con los filtros.</td></tr>
            ) : filasFiltradas.map(f => {
      ...
      {ajusteModal && (
        <AjusteStockModal
          fila={ajusteModal.fila}
          locaciones={locaciones}
          stockPorLoc={stockPorLoc}
          onClose={() => setAjusteModal(null)}
          onSaved={() => { setAjusteModal(null); load(); }}
        />
      )}
...
function AjusteStockModal({ fila, locaciones, stockPorLoc, onClose, onSaved }) {
  ...
  return (
    <div onClick={ev => ev.target === ev.currentTarget && onClose()}
      style={{ position: "fixed", inset: 0, zIndex: 1200, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: B.navyMid, borderRadius: 16, padding: 24, width: 540, maxWidth: "100%", border: `1px solid ${B.warning}44` }}>
        <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 4, color: B.warning }}>🔧 Ajustar Stock</h3>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", fontWeight: 600, textTransform: "uppercase", display: "block", marginBottom: 4 }}>
            Justificación <span style={{ color: B.danger }}>*</span>
          </label>
          <textarea value={motivo} onChange={e => setMotivo(e.target.value)}
            placeholder="Ej: Conteo físico encontró 3 unidades menos. Confirmado por gerente bar."
            style={{ width: "100%", padding: "10px 12px", borderRadius: 8, background: B.navy, border: `1px solid ${B.navyLight}`, color: "#fff", fontSize: 13, minHeight: 80, resize: "vertical", fontFamily: "inherit", boxSizing: "border-box" }} />
          <div style={{ fontSize: 11, color: motivo.trim().length >= 10 ? B.success : "rgba(255,255,255,0.4)", marginTop: 4 }}>
            {motivo.trim().length}/10 caracteres mínimos
          </div>
        </div>

        {err && <div style={{ marginBottom: 12, padding: 10, background: "rgba(239,68,68,0.15)", color: "#fca5a5", borderRadius: 8, fontSize: 12 }}>{err}</div>}

        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} disabled={saving}
            style={{ flex: 1, padding: 11, borderRadius: 8, border: `1px solid ${B.navyLight}`, background: "transparent", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
            Cancelar
          </button>
          <button onClick={aplicar} disabled={saving || motivo.trim().length < 10}
            style={{ flex: 2, padding: 11, borderRadius: 8, border: "none", background: B.warning, color: B.navy, cursor: "pointer", fontSize: 13, fontWeight: 700, opacity: (saving || motivo.trim().length < 10) ? 0.5 : 1 }}>
