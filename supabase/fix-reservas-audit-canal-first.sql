-- Fix: el trigger de audit_reserva prefería NEW.source antes que NEW.canal.
-- Como `source` tiene default 'web', toda reserva creada desde el admin se loggeaba
-- como "Creada vía web" aunque el canal real fuera "WhatsApp", "B2B", "Cortesía", etc.
-- Solución: invertir el orden del COALESCE para que canal mande.
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    -- canal es el campo real del origen del cliente (WhatsApp, B2B, WEB, Cortesía...).
    -- source es metadata técnica que tiene default 'web' aunque la reserva la cree el admin.
    -- Priorizamos canal para que el log refleje el origen real.
    v_notas := 'Creada vía ' || COALESCE(NEW.canal, NEW.source, 'sistema');
