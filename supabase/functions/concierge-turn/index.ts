...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const confirmHint = isConfirm ? [
      ...
      "\n### ORDEN OBLIGATORIO DE ACCIÓN ###",
      "ANTES de escribir la primera palabra al cliente en ESTA conversación, DEBES llamar `get_customer_reservations`. Sin excepciones.",
      "Si el cliente hace CUALQUIER pregunta relacionada a horario, hora de llegada, embarcación, fecha, tipo de pasadía o pax — la respuesta ya está en el resultado de esa tool. Úsala. NO le preguntes 'a qué hora tienes tu salida' ni menciones los horarios genéricos (8:30/10:00/11:30) — eso es información que el sistema ya te dio.",
      "\nPuedes:",
      "- Contarle detalles de sus reservas activas usando los campos EXACTOS del tool (hora_salida_lancha, hora_llegada_bodeguita, tipo, pax, saldo)",
      "- Dar información general de Atolón (punto de encuentro, impuesto de muelle, duración trayecto, políticas — TODO desde el Knowledge Base de arriba)",
      ...
      "\nREGLAS DURAS:",
      "- Anticipación al muelle: SIEMPRE 30 min antes de la salida (usa `hora_llegada_bodeguita` del tool — ya está calculada). Nunca digas '20 min' ni otros números.",
      "- Punto de encuentro, puerta, impuesto de muelle, duración del trayecto: USA EL KNOWLEDGE BASE. Nunca inventes ni asumas.",
      "- Si el KB no tiene un dato específico, dilo claramente en vez de inventar.",
      "- Para pasadías after_island (llega_en_embarcacion_propia=true): el cliente llega en su propia lancha directamente al muelle de Atolón — no pasa por la Bodeguita.",
      "- Si el cliente tiene 0 reservas activas: ofrécele ayudarlo a crear una nueva.",
      "- Si tiene múltiples: listalas brevemente y pregunta de cuál necesita ayuda.",
    ].filter(Boolean).join("\n") : "";
    const system = [
